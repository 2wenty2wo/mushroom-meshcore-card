...
Remote nodes show a Tile-style header, online state, last-seen age, RSSI, SNR, available noise-floor data, battery percentage and voltage, sent/received traffic, uptime, and optional temperature. Repeaters retain their extended diagnostics, location, telemetry, and the complete MeshCore HA 2.9 traffic diagnostics under a collapsed **Details** control, organized into Device, Network, Radio, Network Traffic, Airtime, Message Rates, Reliability, and Telemetry categories. Hubs share the same body primitives: a battery block, hardware/firmware quick chips, and RF, location, MQTT, and other diagnostics under the same **Details** control. Offline devices collapse to their identity and last-seen status with a badge on the icon, while their card surface fills the row allocated by a Sections dashboard.
...
The editor writes a complete `chip_layout`; chips omitted from all three YAML lists are treated as hidden. Existing configurations without `chip_layout` retain their current quick-chip behavior, including the legacy `hide_quick_stats` and `show_firmware` options. Detail chips keep their configured order and are separated into semantic categories; categories with no visible valid metrics are omitted. The neighbour count and dialog include only neighbours provably heard within the integration's rolling 48-hour window. The top neighbour chip shows the icon and count only, and selecting it opens the recent-neighbours dialog. `show_neighbors: false` hides the chip and disables the dialog, while `max_neighbors` limits the visible dialog rows without changing the reported total.
...
With MeshCore HA 2.9 or later, messages received while the card is loaded can be enriched from the integration's native messaging events with subtle hop-count, path-hash, automatically detected one-, two-, or three-byte path width, and regional-scope details beneath the message. Selecting the Path pill opens a compact popup containing every unique live route reported for that message. The popup resolves path tokens against contacts known to the selected channel's hub on a best-effort basis, while keeping unknown and ambiguous matches explicit.

This is route and reception metadata, not authenticated routing: channel messages are unverified, path tokens are truncated public-key hashes, and displayed repeater identities are locally inferred hints. Incoming packets do not contain a reversible scope name. To display a name such as `au`, add `au` under **MeshCore → Configure → Global Settings → Flood Scope Allowlist**; MeshCore HA can then emit `flood_scope: "#au"`, which the card displays as `au`. When a message is region-scoped but no single exact name is available, the card continues to show **Regional**.

Logbook does not retain the native route fields, so the card stores matched routing metadata in Home Assistant's per-user frontend storage. This keeps pills and popup routes across dashboard reloads, browser restarts, and other devices using the same account. The shared cache uses bounded retention independently of each card's `hours_to_show` and `max_messages`; those settings continue to control only the Logbook rows visible in that card. Messages received while no channel-card instance was active cannot be backfilled. Storage failures are optional and fall back to live-only enrichment. Home Assistant may reject arbitrary `subscribe_events` subscriptions for non-administrator users; in that case, route details stay hidden while Logbook messages continue normally.
