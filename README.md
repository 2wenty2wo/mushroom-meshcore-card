...
Logbook does not retain the native route fields, so the popup and inline routing details are live-session enrichment only. Older rows loaded after a dashboard reload cannot be backfilled and remain unchanged. Home Assistant may reject arbitrary `subscribe_events` subscriptions for non-administrator users; in that case, the optional route details stay hidden while Logbook messages continue normally.
